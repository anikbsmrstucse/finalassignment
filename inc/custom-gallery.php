...
// custom gallery functionality implement in custom gallery post type
/************************************************************/
...
function property_gallery_add_metabox()
{
	add_meta_box(
		...
		'recent_work', // Change post type name
		'normal',
		'core'
	);
}
...
function property_gallery_save($post_id)
{
	...
	// Correct post type
	// if ( 'custom_gallery' != $_POST['post_type'] ) // here you can set the post type name
	// 	return;

	// Check if the 'post_type' key exists in $_POST
    //change the gallery post type name
	if (isset($_POST['post_type']) && 'recent_work' != $_POST['post_type']) {
		return;
	}
